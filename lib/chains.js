...
chain.endLoop = function() {
	var self = this;
	this.promise = this.promise.then(function(win) { self.__endLoop(); }, function(fail) { console.log('Failure caught in "endLoop" cleanup function', fail); });
}
//Deprecated
...
chain.end = function (func) {
	...
	this.promise = this.promise.then(function(win) { 
		self.__end(); 
	}, function(fail) { console.log('Failure caught in "end" cleanup function', fail); });
	if (typeof func !== 'function') {
